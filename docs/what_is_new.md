...
### PBFT
PBFT是一种BFT类共识算法，可容忍不超过三分之一的故障节点和作恶节点，可达到最终一致性。可以从以下几方面验证共识特性。<br/><br/>
1. 交易类型：系统类交易（出厂提供的相关操作，诸如权限、CNS、节点管理、群组管理等）、业务侧交易（业务为实现某种功能编写的合约等交易）均能共识。
2. leader节点：
    - 实时压测过程中，各个共识节点轮流作为leader节点。
    - 交易池内堆积的历史交易，各个共识节点轮流作为leader节点。
    - 多个客户端发送到相同/不同直连节点，轮流作为leader。
    - 某共识节点异常，剩余正常节点中，其中一个节点被轮询2次作为leader节点。
    - 观察节点不参与打包，会同步区块。

3. 多群组架构中，各个群组的的共识互不干扰。
4. 3f+1原则：
    - 3节点环境，1个节点异常，链异常。
