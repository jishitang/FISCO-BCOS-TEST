...
可以从以下几方面验证共识特性。<br/>
* 1. 交易类型：系统类交易（提供的相关系统操作，诸如权限、CNS、节点管理、群组管理等）、业务侧交易（应用侧为实现某种功能编写的合约等交易）均能共识。
* 2. 共识过程（PBFT）：PBFT可以在少数节点作恶(如伪造消息)场景中达成共识，它采用签名、签名验证、哈希等密码学算法确保消息传递过程中的防篡改性、防伪造性、不可抵赖性。
...
* 3. 共识过程（Raft）：在Raft算法中，每个网络节点只能如下三种身份之一：Leader、Follower以及Candidate。Leader主要负责与外界交互，由Follower节点选举而来，在每一次共识过程中有且仅有一个Leader节点，由Leader全权负责从交易池中取出交易、打包交易组成区块并将区块上链。Follower以Leader节点为准进行同步，并在Leader节点失效时举行选举以选出新的Leader节点。Candidate是Follower节点在竞选Leader时拥有的临时身份。
    ...
    - 选举leader时，先到先得（即先收到majority投票的节点，赢得选举，成为新leader，可通过网络延迟模拟）。
* 4. 共识过程（rPBFT）：rPBFT算法每轮共识流程仅选取若干个节点执行PBFT共识流程出块，并根据区块高度周期性地替换共识节点，保障系统安全。包括epoch_sealer_num、epoch_block_num两个重要的参数，包括共识节点、验证节点两种节点类型。
    - epoch_sealer_num等于节点总数，交易处理正常，不存在动态替换共识节点。
    - epoch_sealer_num小于节点总数，交易处理正常，每出epoch_block_num个块后，替换一个共识节点。
    - 共识节点异常：若满足3f+1，交易能正常处理。若不满足3f+1，停止处理交易。故障节点恢复后，业务恢复。
    - 
    - 
...
* 5. 容错（不同共识算法容错的节点个数不相同，若故障节点过多导致链异常后，当恢复部分节点满足容错规则后，链应能自动恢复）：
...
* 6. 多群组架构中，各个群组的共识算法互不干扰（各群组可以采用相同/不同共识算法）。
