...
PBFT是一种BFT类共识算法，可容忍不超过三分之一的故障节点和作恶节点，可达到最终一致性。<br/>
Raft共识算法: CFT类算法, 可容忍一半故障节点，不能防止节点作恶，可达到一致性。<br/>
...
2. 共识过程（PBFT）：
####leader节点：
    - 实时压测过程中、交易池内堆积的历史交易，各个共识节点轮流作为leader节点。
...
####视图共识：
    ...
    - 
3. 共识过程（Raft）：
    - 压测过程中，leader节点正常，一直由该节点作为leader。
    - 停止leader节点暂停，更新leader。
    - leader进程暂停，由新leader打包。
    - 
...
6. 容错（不同共识算法容错的节点个数不相同，若故障节点过多导致链异常后，当恢复部分节点满足容错规则后，链应自动恢复）：
####3f+1（PBFT）：
    - 3节点环境，1个节点异常，链异常。
...
####2f+1（Raft）：
    - 3节点环境，1个节点异常，链正常。
    - 4节点环境，若1个节点异常，链能正常工作。若2个节点异常，链异常。
    - 5节点环境，若1/2个节点异常，链能正常工作。若3个节点异常，链异常。


11. 拜占庭容错，如何模拟？？？？
12. 多群组架构中，各个群组的共识互不干扰（各群组相同共识算法、不同共识算法）。
