...
## 同步
同步，作为节点共识的辅助，是区块链系统的重要特性之一。此处所述同步包括状态同步和交易同步。
### 状态同步
状态同步是为了让区块链各节点的状态保持在最新。只有拥有最新区块的节点，才能参与群组共识。
...
停止节点一段时间、新扩容节点加入群组、
链异常，部分节点不能同步
新组员同步：离节点不同步，游离节点设置为观察节点开始同步，设置为共识节点也会同步
多账本：多个群组同时同步、一个群组正常共识，一个群组同步
节点升级后同步历史数据。
不同存储类型同步，sql向sql同步，sql向rocksdb同步，rocksdb向rocksdb同步，rocksdb向sql同步。
