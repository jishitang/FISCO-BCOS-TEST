...
#### 共识节点
共识节点即可以参与共识的节点，拥有群组的所有数据（搭链时默认都生成共识节点）。观察节点、游离节点均可被添加为共识节点。console提供了addSealer命令将节点设置为共识节点，getSealerList命令查看当前group的共识节点列表。
针对console的addSealer操作，测试过程中需要验证到如下细节（前提条件：节点被添加到的group正常工作）：
...
+ 待添加观察节点未同步到链上最新数据，addSealer成功后会继续同步数据不能正常工作，待同步到最新数据后才能正常工作。
+  
...
#### 观察节点
观察节点：不参与共识，但能实时同步链上的数据。<br>

#### 游离节点
游离节点：已启动，待加入群组的节点，不能获取链上的数据。
