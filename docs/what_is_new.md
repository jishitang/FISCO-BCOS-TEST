...
* 4. rPBFT：rPBFT算法每轮共识流程仅选取若干个节点执行PBFT共识流程出块，并根据区块高度周期性地替换共识节点，保障系统安全。包括epoch_sealer_num、epoch_block_num两个重要的参数，包括共识节点、验证节点两种节点类型。
    - epoch_sealer_num等于节点总数，交易处理正常，不存在动态替换共识节点。
    - epoch_sealer_num小于节点总数，交易处理正常，epoch_block_num个块内，各共识节点轮流出块。每出epoch_block_num个块后，替换一个共识节点。
    - 共识节点异常：共识节点范围内，若满足3f+1，交易能正常处理；若不满足3f+1，停止处理交易。故障节点恢复后，业务恢复。
    - 验证节点部分异常：共识节点替换时，能替换到正常的节点？
    - 验证节点全部异常：交易正常处理。当替换共识节点时，由于验证节点全部异常，替换失败，当前共识节点列表中，其中一个节点作为leader打包两次。下一轮替换共识节点时，回到上一轮的共识节点列表。
    - 验证节点全部异常：交易正常处理。当替换共识节点时，由于验证节点全部异常，替换失败（此时不替换？？？？？），当前共识节点列表中，其中一个节点作为leader打包两次（此时停止一个leader节点，链异常？？？？）。下一轮替换共识节点时，回到上一轮的共识节点列表。
    - 
    - 
