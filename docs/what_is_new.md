...
## 节点管理
节点是区块链系统工作的最小单元，目前FISCO BCOS系统中的节点有3种类型（游离节点、观察节点以及共识节点，3种节点类型可相互转换），2种状态（正常场景下包括启动状态、停止状态）。<br/><br/>
...
+ 客户端（如java-sdk）配置的直连节点为共识节点，该直连节点能接受客户端发过来的交易。<br/><br/>
...
同样的，为避免破坏链的正常工作形态，在节点退出时，一般按照如下顺序操作：节点removeNode退出群组、停止节点或设置CA黑白名单等退出网络。<br/><br/>

不同的节点类型有不同的表现形式，在实际压测过程中，可以让节点在3种类型之间来回转换，观察各种类型的节点表现是否符合预期。<br/><br/>
