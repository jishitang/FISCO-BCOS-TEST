...
## 同步
同步，作为节点共识的辅助，是区块链系统的重要特性之一，包括状态同步和交易同步。
### 状态同步
状态同步是为了让区块链各节点的区块保持在群组的最新数据。只有拥有最新区块的节点，才能正常参与群组的共识。如下列举一些常见的同步场景：
1. 新组员同步：新组员加入到群组后，自身块高为0，跟其他组员块高比较后开始区块同步，同步结束后切换自身状态，开启交易同步，开启共识。<br/><br/>
2. 同步时负载均衡：节点A数据落后，节点A会向组内所有正常节点发送区块下载请求包，确保各节点负载均衡。可以在节点日志中搜索如下关键字统计。
```Bash
[fangli@VM_108_28_centos log]$ cat log_2021042210.40.log |grep -w 'Batch blocks for sending'|grep 'g:2' | wc -l
39563
```
3. 不同存储类型的节点之间同步：
RocksDB模式-->RocksDB模式,RocksDB模式-->MySQL模式,RocksDB模式-->Scalable模式。
MySQL模式-->RocksDB模式，MySQL模式-->MySQL模式，MySQL模式-->Scalable模式。
Scalable模式-->RocksDB模式，Scalable模式-->MySQL模式，Scalable模式-->Scalable模式。
4. 
...
多账本：多个群组同时同步、一个群组正常共识，一个群组同步
节点升级后同步历史数据。

大数据量的同步：
已有链已经运行了很长一段时间，已经落盘的块高达到百万甚至千万级别，这个时候，如果新增节点，此节点进行状态同步需要花费很长的时候。我们就需要观察是否在长时间的状态同步中，会出现异常的情况。
网络不稳定时，同步是否能持续，一直同步到最新数据。
